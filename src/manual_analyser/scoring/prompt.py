    ...
    for criterion in mode_config.criteria:
        if criterion.is_llm:
            try:
                ...
                logger.error("Failed to build prompt for criterion '%s': %s", criterion.id, e, exc_info=True)
    return packages
...
def _fetch_field_values(
    ...
    try:
        ...
        # Fetch sections — return as summary
        if sections_fields:
        ...
        # Fetch chord progressions — one per section, joined
        if chord_fields:
